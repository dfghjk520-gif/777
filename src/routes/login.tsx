...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in to play the Seven77 daily lottery." },
    ],
  }),
  component: LoginPage,
});
    ...
    <PageShell>
      <section className="mx-auto max-w-md px-4 pt-16 pb-20">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Sign in to play today&apos;s draw.</p>
        </div>

        <form onSubmit={submit} className="mt-8 glass-strong rounded-3xl p-6 space-y-4">
          <button
            type="button"
            onClick={() => { setUsername("Player"); navigate({ to: "/play" }); }}
          ...
          <p className="text-center text-xs text-muted-foreground">
            New here? <Link to="/login" className="text-foreground underline underline-offset-4">Create an account</Link>
          </p>
        </form>

        <p className="mt-6 text-center text-xs text-muted-foreground">
          Demo mode · No real authentication. Real auth ships with Lovable Cloud.
        </p>
      </section>
    </PageShell>
  );
